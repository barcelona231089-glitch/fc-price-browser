...
export const V1063_BOOTSTRAP_VERSION = '10.63-bootstrap-1';
...
import {
  ...
  if (!source.includes('v10.63 FUT.GG trading dimensions')) {
    source = source.replace(
      ...
      `      position: card.position,
      club: card.club,
      // v10.63 FUT.GG trading dimensions
  ...
  if (!source.includes('v10.63 adaptive cycle')) {
    source = source.replace(
      `  await persistDiscordSignalMarketConfirmations(rows, brainWork);`,
      `  await persistDiscordSignalMarketConfirmations(rows, brainWork);

  // v10.63 adaptive cycle: market regime + catalyst graph + source learning +
  ...
  if (!source.includes('v10.63 Gemini final recalibration')) {
    source = source.replace(
      `      applyDecisionPerformanceCalibration(geminiCandidate.row, decisionPerformanceCache);`,
      `      applyDecisionPerformanceCalibration(geminiCandidate.row, decisionPerformanceCache);
      // v10.63 Gemini final recalibration
  ...
  if (!source.includes('v10.63 final per-row decision')) {
    source = source.replace(
  ...
  if (!source.includes('v10.63 FUTBIN + FC26 recalibration')) {
    source = source.replace(
      'await automaticTraderBrain(latestTradingRows, built.brainWork);',
      `// v10.63 FUTBIN + FC26 recalibration
      for (const row of latestTradingRows) {
        const work = built.brainWork.get(String(row.eaId));
        adaptiveV1062RecalibrateCachedRow(row, work, {
          gameYear: GAME_YEAR,
  ...
  if (!source.includes('v10.63 CPU governor market gate')) {
    source = source.replace(
  ...
  if (HA_ENABLED && !haIsLeader()) return;
  if (monitoringBusy) return;
  // v10.63 CPU governor market gate
  ...
  if (!source.includes('v10.63 CPU governor metadata gate')) {
    source = source.replace(
      `    () => {
      ensureUniverse(true).catch(`,
      `    () => {
      // v10.63 CPU governor metadata gate
  ...
  if (!source.includes('v10.63 BUY/SELL-only Discord gate')) {
    source = source.replace(
  ...
  if (HA_ENABLED && !haIsLeader()) throw new Error("HA_STANDBY: Discord send blocked on passive instance");`,
      `async function sendDiscordPayload(payload) {
  // v10.63 BUY/SELL-only Discord gate. No health/HA/system notices are public.
  if (!adaptiveV1062DiscordPayloadAllowed(payload)) return { ok: false, skipped: "v10.63_buy_sell_only" };
  if (HA_ENABLED && !haIsLeader()) throw new Error("HA_STANDBY: Discord send blocked on passive instance");`
    );
  }
  ...
  // v10.63 automatic low-rate FC26 FUTBIN backfill after the first healthy market snapshot.
  if (!source.includes('v10.63 automatic FC26 backfill')) {
    source = source.replace(
      /(latestTradingRows\s*=\s*built\.rows;\s*\n\s*latestFutbinFallbackRows\s*=\s*\[\];)/,
      `$1
      // v10.63 automatic FC26 backfill
      if (dbEnabled && !globalThis.__fcV1063BackfillStarted && latestTradingRows.length >= 100) {
        globalThis.__fcV1063BackfillStarted = true;
        startFc26Backfill({ rows: latestTradingRows, pool, gameYear: "26", limit: 300 })
          .then(result => console.log(\`[v10.63] FC26 FUTBIN backfill gestartet: \${JSON.stringify(result)}\`))
          .catch(error => console.error("[v10.63] FC26 FUTBIN backfill start error:", error));
      }`
    );
  }

  // v10.63: build the FC26 season-memory summary from every historical
  // PostgreSQL price point already available. Runs in background, never blocks boot.
  if (!source.includes('v10.63 FC26 season bootstrap')) {
    source = source.replace(
      `    await initDb();
  } catch (error) {`,
      `    await initDb();
    // v10.63 FC26 season bootstrap
    if (dbEnabled) {
      bootstrapFc26SeasonMemory({ pool, gameYear: "26" })
        .then(result => console.log(\`[v10.63] FC26 Season Memory bootstrap: \${JSON.stringify(result)}\`))
        .catch(error => console.error("[v10.63] FC26 Season Memory bootstrap error:", error));
    }
  } catch (error) {`
    );
  }
  ...
  try {
    ...
    res.status(500).json({ ok: false, version: "10.63-adaptive-market-intelligence", error: String(error) });
  }
});
...
app.get("/api/trader-brain/status", (req, res) => {`
  ...
  // v10.63 FUTBIN-public + FC26 backfill/status endpoints.
...
app.get("/api/trader-brain/status", (req, res) => {`
  ...
  // Version labels.
  source = source.replaceAll('10.56-public-leak-learning-brain', '10.63-adaptive-market-intelligence');
  source = source.replace(
    'FC Trading Intelligence v10.61 AI Direction Consensus + Sheriff Multi-Source (FC${GAME_YEAR}) running on ${port}',
    'FC Trading Intelligence v10.63 Adaptive Market Intelligence + FUTBIN Gap-Fill + FC26 Memory (FC${GAME_YEAR}) running on ${port}'
  );
...
export async function load(url, context, defaultLoad) {
  ...
  if (!patched?.source || typeof patched.source !== 'string') {
    throw new Error('[v10.63] In-memory patch returned no server source.');
  }

  console.log(
    patched.changed
      ? '[v10.63] server.js in-memory gepatcht: FUTBIN Gap-Fill + FC26 Memory + BUY/SELL only.'
      : '[v10.63] server.js bereits kompatibel; in-memory Loader aktiv.'
  );
