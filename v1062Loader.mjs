...
export async function load(url, context, defaultLoad) {
  ...
  if (!patched?.source || typeof patched.source !== 'string') {
    throw new Error('[v10.62.1] In-memory patch returned no server source.');
  }

  console.log(
    patched.changed
      ? '[v10.62.1] server.js in-memory gepatcht (Hostless read-only kompatibel).'
      : '[v10.62.1] server.js bereits v10.62-kompatibel; in-memory Loader aktiv.'
  );
