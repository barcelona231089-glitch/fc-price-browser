...
export const ADAPTIVE_BRAIN_VERSION = '10.64-complete-market-memory';
...
function futbinWindowAdjustment(row, side = 'BUY') {
  const w = row?.futbinHistoryWindows || row?.futbinPublic?.historyWindows;
  if (!w) return 0;
  const c15 = Number(w?.m15?.changePct);
  const c1h = Number(w?.h1?.changePct);
  const c6h = Number(w?.h6?.changePct);
  const c24 = Number(w?.h24?.changePct);
  const v1h = Number(w?.h1?.volatilityPct);
  let buy = 0;
  if (Number.isFinite(c15)) buy += clamp(c15 / 2.5, -4, 4);
  if (Number.isFinite(c1h)) buy += clamp(c1h / 3.5, -5, 5);
  if (Number.isFinite(c6h)) buy += clamp(c6h / 6, -4, 4);
  if (Number.isFinite(c24)) buy += clamp(c24 / 10, -3, 3);
  if (Number.isFinite(v1h) && v1h > 8) buy -= Math.min(4, (v1h - 8) / 3);
  buy = clamp(buy, -10, 10);
  return side === 'SELL' ? clamp(-buy * 0.75, -8, 8) : buy;
}


function futbinMarketOverviewAdjustment(row, side = 'BUY') {
  const overview = row?.futbinMarketOverview;
  const index = row?.futbinRatingIndex;
  if (!overview && !index) return 0;
  const momentum = Number(row?.futbinMarketMomentum ?? overview?.marketMomentum);
  const indexMove = Number(index?.consoleChangePct);
  let buy = 0;

  if (Number.isFinite(momentum)) {
    if (momentum >= 65) buy += clamp((momentum - 60) / 8, 0, 3);
    else if (momentum <= 35) buy -= clamp((40 - momentum) / 7, 0, 3);
  }
  if (Number.isFinite(indexMove)) buy += clamp(indexMove / 2.5, -4, 4);

  buy = clamp(buy, -6, 6);
  return side === 'SELL' ? clamp(-buy * 0.7, -5, 5) : buy;
}

function sourceFreshnessAdjustment(row) {
  const freshness = row?.futbinPriceComparison?.freshness || row?.futbinPublic?.priceComparison?.freshness;
  if (!freshness) return 0;
  const diff = Number(freshness.futbinMinusFutggMinutes);
  const priceDiff = Number(row?.futbinPriceComparison?.futbinVsFutggPct ?? row?.futbinPublic?.priceComparison?.futbinVsFutggPct);
  if (!Number.isFinite(diff)) return 0;
  let adj = 0;
  if (freshness.fresher === 'FUTBIN' && diff >= 5) adj += 1;
  if (freshness.fresher === 'FUTGG' && diff <= -5) adj += 0.5;
  if (Number.isFinite(priceDiff) && Math.abs(priceDiff) >= 12) adj -= 4;
  else if (Number.isFinite(priceDiff) && Math.abs(priceDiff) >= 7) adj -= 2;
  return clamp(adj, -5, 2);
}

function historicalCatalystAdjustment(row, catalyst, side = 'BUY') {
  if (!catalyst || catalyst === 'NONE') return 0;
  const profile = row?.fc26GlobalMemory?.eventPatterns?.[catalyst];
  if (!profile || Number(profile.events || 0) < 2) return 0;
  const h1 = Number(profile.avgImpact1hPct);
  const h6 = Number(profile.avgImpact6hPct);
  const h24 = Number(profile.avgImpact24hPct);
  const values = [];
  if (Number.isFinite(h1)) values.push([h1, 0.45]);
  if (Number.isFinite(h6)) values.push([h6, 0.35]);
  if (Number.isFinite(h24)) values.push([h24, 0.20]);
  if (!values.length) return 0;
  const weighted = values.reduce((sum, [v,w]) => sum + v*w, 0) / values.reduce((sum,[,w]) => sum+w,0);
  const sampleWeight = Math.min(1, Number(profile.events || 0) / 12);
  const buy = clamp(weighted * 1.2 * (0.5 + sampleWeight * 0.5), -8, 8);
  return side === 'SELL' ? clamp(-buy * 0.8, -7, 7) : buy;
}


function currentSeasonPhaseKey(gameYear = '26', now = Date.now()) {
  const yy = Number(String(gameYear).replace(/\D/g, '').slice(-2));
  const endYear = 2000 + (Number.isFinite(yy) ? yy : 26);
  const start = Date.UTC(endYear - 1, 8, 1);
  const days = Math.max(0, Math.floor((now - start) / 86_400_000));
  const phase = Math.max(1, Math.min(14, 1 + Math.floor(days / 30)));
  return `P${String(phase).padStart(2,'0')}`;
}

function seasonPhaseAdjustment(row, gameYear = '26', side = 'BUY') {
  const key = currentSeasonPhaseKey(gameYear);
  const p = row?.fc26GlobalMemory?.phasePatterns?.[key];
  if (!p || Number(p.days || 0) < 5) return 0;
  const avg = Number(p.avgMarketChangePct);
  const crash = Number(p.crashSharePct);
  const recovery = Number(p.recoverySharePct);
  const supply = Number(p.supplySharePct);
  let buy = 0;
  if (Number.isFinite(avg)) buy += clamp(avg * 1.5, -4, 4);
  if (Number.isFinite(crash) && crash >= 15) buy -= 2;
  if (Number.isFinite(supply) && supply >= 25) buy -= 1.5;
  if (Number.isFinite(recovery) && recovery >= 15) buy += 1.5;
  buy = clamp(buy, -6, 6);
  return side === 'SELL' ? clamp(-buy * 0.7, -5, 5) : buy;
}
...
function seasonMemoryAdjustment(row, side = 'BUY') {
  ...
  const volatility = Number(m.dailyVolatilityPct);
  const drawdown = Number(m.maxDrawdownPct);
  ...
  buy *= 0.45 + depth * 0.55;
  if (side === 'SELL') return clamp(-buy * 0.8, -8, 8);
  return clamp(buy, -9, 9);
}
...
function chooseDataNeeds(row, regime, catalyst) {
  const futgg = new Set([
    'liveBin','priceStatus','1m','5m','15m','1h','24h','cardVersion','tradeability',
    'inPacks','demand','usage','popularity','supply','club','league','nation','position'
  ]);
  const futbin = new Set([
    'games','gpg','currentPrice','priceUpdatedAt','priceRange','prp','trend',
    'listedFor','soldFor','soldStatus','salesHistory','soldPriceMedian','soldPriceAverage',
    'sellThrough','saleVelocity','listingVelocity','liquidity','priceStability','undercutRate',
    'relistEstimate','timeToSaleEstimate','priceHistory','5m','15m','1h','6h','24h','7d','30d',
    'versionComparison','promoName','position','alternatePositions','nation','league','club',
    'sourceFreshness','eaAveragePrice','sourceCrossCheck'
  ]);

  if (['SBC_FODDER_PUSH', 'SUPPLY_WAVE'].includes(regime) || ['SBC', 'MARQUEE_MATCHUPS', 'REWARDS'].includes(catalyst)) {
    ['rating','momentum'].forEach(x => futgg.add(x));
    ['marketActivity','soldVsUnsoldGap'].forEach(x => futbin.add(x));
  }
  if (['PROMO_HYPE', 'RECOVERY', 'CRASH'].includes(regime) || ['PROMO', 'EVO', 'OUT_OF_PACKS'].includes(catalyst)) {
    ['mostUsed','communityUsage','proUsage','momentum'].forEach(x => futgg.add(x));
    ['popularity','likes','dislikes','historyHighLow'].forEach(x => futbin.add(x));
  }
...
function enrichReason(row, decision) {
  const sales = row?.futbinPublic?.salesMetrics || {};
  const h1 = row?.futbinHistoryWindows?.h1 || row?.futbinPublic?.historyWindows?.h1;
  const h24 = row?.futbinHistoryWindows?.h24 || row?.futbinPublic?.historyWindows?.h24;
  const catalystHistory = decision?.historicalCatalystProfile;
  const parts = [
    `v10.64 ${REGIME_LABELS[decision.regime] || decision.regime}.`,
    ...
    sales?.listingsObserved ? `FUTBIN Listings ${sales.listingsObserved}: ${sales.soldListings} verkauft, ${sales.unsoldListings} nicht verkauft, Sell-through ${sales.sellThroughRate ?? 'n/a'}%.` : null,
    sales?.medianSoldPrice ? `Echter Median-Sale ${sales.medianSoldPrice}, letzter Sale ${sales.lastSalePrice ?? 'n/a'}.` : null,
    sales?.liquidityScore != null ? `Liquidität ${sales.liquidityScore}/100, Sale-Velocity ${sales.saleVelocityPerHour ?? 'n/a'}/h, Stabilität ${sales.priceStabilityPct ?? 'n/a'}%.` : null,
    sales?.estimatedTimeToSaleMinutes != null ? `geschätzte Verkaufszeit ${sales.estimatedTimeToSaleMinutes} Min, Relists ${sales.estimatedRelistCount ?? 0}.` : null,
    row?.futbinGamesPlayedConsole ? `FUTBIN Games ${row.futbinGamesPlayedConsole}.` : null,
    Number.isFinite(Number(h1?.changePct)) ? `FUTBIN 1h ${Number(h1.changePct).toFixed(2)}%.` : null,
    Number.isFinite(Number(h24?.changePct)) ? `24h ${Number(h24.changePct).toFixed(2)}%.` : null,
    row?.futbinPriceComparison?.freshness?.fresher ? `Frischere Quelle: ${row.futbinPriceComparison.freshness.fresher}.` : null,
    row?.futbinMarketMomentum != null ? `FUTBIN Markt-Momentum ${row.futbinMarketMomentum}/100 (${row.futbinMarketMood ?? 'n/a'}), ${row.futbinRatingIndexKey ?? 'Index'} ${row.futbinRatingIndex?.consoleChangePct ?? 'n/a'}%.` : null,
    row?.fc26SeasonMemory?.days ? `FC26-Memory ${row.fc26SeasonMemory.days} Tage, Return ${row.fc26SeasonMemory.seasonReturnPct ?? 'n/a'}%, Drawdown ${row.fc26SeasonMemory.maxDrawdownPct ?? 'n/a'}%.` : null,
    catalystHistory?.events >= 2 ? `FC26 ${decision.catalyst}: ${catalystHistory.events} historische Events, Ø 1h ${catalystHistory.avgImpact1hPct ?? 'n/a'}%, 6h ${catalystHistory.avgImpact6hPct ?? 'n/a'}%.` : null,
    row?.fc26GlobalMemory?.phasePatterns?.[decision?.scoreAdjustments?.seasonPhaseKey]
  ...
  return normalizeText(...parts).slice(0, 1900);
}
...
function applyDecision(row, work, context) {
  ...
  const historicalCatalystProfile = row?.fc26GlobalMemory?.eventPatterns?.[catalyst] || null;
  const futbinWindowBuy = futbinWindowAdjustment(row, 'BUY');
  const futbinWindowSell = futbinWindowAdjustment(row, 'SELL');
  const freshnessAdj = sourceFreshnessAdjustment(row);
  const marketOverviewBuyAdj = futbinMarketOverviewAdjustment(row, 'BUY');
  const marketOverviewSellAdj = futbinMarketOverviewAdjustment(row, 'SELL');
  const catalystBuyAdj = historicalCatalystAdjustment(row, catalyst, 'BUY');
  const catalystSellAdj = historicalCatalystAdjustment(row, catalyst, 'SELL');
  ...
  const decision = {
    ...
    futbinPublic: row?.futbinPublic ? {
      ...
      historyWindows: row.futbinHistoryWindows ?? row.futbinPublic.historyWindows ?? null,
      priceComparison: row.futbinPriceComparison ?? row.futbinPublic.priceComparison ?? null,
      promoName: row.futbinPromoName ?? row.futbinPublic.promoName ?? null,
      availableVersions: row.futbinAvailableVersions ?? row.futbinPublic.availableVersions ?? [],
      marketOverview: row.futbinMarketOverview ?? null,
      ratingIndexKey: row.futbinRatingIndexKey ?? null,
      ratingIndex: row.futbinRatingIndex ?? null
    } : null,
    fc26SeasonMemory: row?.fc26SeasonMemory ?? null,
    fc26GlobalMemory: row?.fc26GlobalMemory ?? null,
    historicalCatalystProfile,
    scoreAdjustments: {
      futbinWindowBuy: Number(futbinWindowBuy.toFixed(3)),
      futbinWindowSell: Number(futbinWindowSell.toFixed(3)),
      sourceFreshness: Number(freshnessAdj.toFixed(3)),
      marketOverviewBuy: Number(marketOverviewBuyAdj.toFixed(3)),
      marketOverviewSell: Number(marketOverviewSellAdj.toFixed(3)),
      catalystBuy: Number(catalystBuyAdj.toFixed(3)),
      catalystSell: Number(catalystSellAdj.toFixed(3)),
      seasonPhaseBuy: Number(phaseBuyAdj.toFixed(3)),
      seasonPhaseSell: Number(phaseSellAdj.toFixed(3)),
      seasonPhaseKey
    },
  ...
  row.aiModelUsed = `${String(row.aiModelUsed || 'Quantitative Core')} + v10.64 Adaptive`;
  row.aiAdaptiveV1062 = decision; // compatibility alias
  row.aiAdaptiveV1064 = decision;
  ...
  if (work?.input && typeof work.input === 'object') {
    ...
    work.input.adaptiveV1062 = {
      ...
      fc26GlobalMemory: decision.fc26GlobalMemory,
      historicalCatalystProfile: decision.historicalCatalystProfile,
      scoreAdjustments: decision.scoreAdjustments
    };
    work.input.adaptiveV1064 = work.input.adaptiveV1062;
  }
...
export async function adaptiveV1062Status({ pool = null, gameYear = '26' } = {}) {
  ...
  return {
    ...
    learning: {
      ...
      fc26SeasonPhaseTransferLearning: true,
      fc26CatalystOutcomeLearning: true,
    ...
    dataIntelligence: {
      ...
      futbinCompleteGapFill: true,
      futbinGamesSalesTrend: true,
      futbinHistoryWindows: ['5m','15m','1h','6h','24h','7d','30d'],
      futbinSourceFreshness: true,
      futbinMarketIndices: true,
      futbinMarketMomentum: true,
      futbinRatingIndexContext: true,
      noProtectionBypass: true
    },
    seasonMemory: {
      externalMemoryCount,
      cycleMemoryCount,
      eventPatternMemory: true,
      regimeCalendarMemory: true,
      cardSeasonStatistics: true,
