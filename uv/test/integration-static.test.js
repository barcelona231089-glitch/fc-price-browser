...
test('v2.3 production status enforces hard 100 slots and sellability-first live evidence', () => {
  ...
  assert.match(uvApp, /const UV_VERSION = '2\.11\.0'/);
...
test('v2.10 demand/market-fit is the primary ranking layer and rating is not the primary ranker', () => {
  ...
  assert.ok(uvApp.includes('marketTradeableHardGuard: true'));
  assert.ok(uvApp.includes('confirmedLiveBinRequired: true'));
  assert.ok(uvApp.includes('confirmTradeableMarketCards'));
