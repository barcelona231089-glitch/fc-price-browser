...
async function loadStatus(){
  try{
    ...
    const fullStatus = `FC${s.gameYear} | FUT.GG ${s.currentCapabilities.futggLivePrices?'OK':'NO'} | Nachfrage ${s.currentCapabilities.futggMostUsedDemand?'OK':'NO'} | DB ${s.databaseConfigured?'OK':'NO'} | Lernen ${s.currentCapabilities.selfLearningPriceSafety?'OK':'NO'} | Bronze ${s.currentCapabilities.bronzeHardBlock?'BLOCK':'?'} | Normal<82 ${s.currentCapabilities.normalCardMinimumRating===82?'BLOCK':'?'} | NR<=82 ${s.currentCapabilities.nonRareDemandGate?'BLOCK':'?'} | Special ${s.currentCapabilities.specialCardPriority?'PRIO':'?'} | Dynamik ${s.currentCapabilities.dynamicMarketPolicy?'OK':'?'} | Promo/Markt ${s.currentCapabilities.promoMarketAdaptive?'OK':'?'} | Trader-Consensus ${s.currentCapabilities.traderConsensusRanker?'OK':'NO'} | Budget-Tier ${s.currentCapabilities.budgetTierAllocator?'OK':'NO'} | Max x${s.currentCapabilities.maxExactCardCopies||'?'} | FUTBIN ${s.futbinParseConfigured?'OK':'NO'} | Games ${s.currentCapabilities.pgpGames?'OK':'NO'} | Sales ${s.currentCapabilities.playerSalesHistory?'OK':'NO'} | v${s.version}`;
    badge.textContent = `FC${s.gameYear} | FUT.GG ${s.currentCapabilities.futggLivePrices?'OK':'NO'} | DB ${s.databaseConfigured?'OK':'NO'} | Promo/Markt ${s.currentCapabilities.promoMarketAdaptive?'OK':'?'} | Consensus ${s.currentCapabilities.traderConsensusRanker?'OK':'NO'} | Tier ${s.currentCapabilities.budgetTierAllocator?'OK':'NO'} | Dynamik ${s.currentCapabilities.dynamicMarketPolicy?'OK':'?'} | v${s.version}`;
...
function renderSummary(data){
  const primary = [
    metric('Budget',`${coins(data.budget)} Coins`),
    metric('Einkauf',`${coins(data.totalBuy)} Coins`,true),
    metric('Slots',`${data.count}${data.dynamicCountReduced?'  |  dynamisch':''}`,true),
    metric('Specials',`${data.specialCount||0} / ${data.count}`),
    metric('Profit 1x',`${coins(data.totalExpectedProfit)} Coins`,true)
  ].join('');
  ...
  const details = [
    ...
    metric('Avg Langfrist-Score',`${Number(data.avgLongTermScore).toFixed(1)}/100`),
    metric('Avg Daten-Sicherheit',`${Number(data.avgConfidence).toFixed(0)}/100`),
    metric('Avg Preisaktivität',`${Number(data.avgActivity).toFixed(0)}/100`),
    metric('Avg Nachfrage',`${Number(data.avgPopularity).toFixed(0)}/100`),
    ...
    metric('Avg Trader-Endgame',`${Number(data.avgPublicTraderEndgameScore||50).toFixed(0)}/100`),
    metric('Avg Trader-Consensus',`${Number(data.avgTraderConsensusScore||50).toFixed(0)}/100`,true),
    metric('Avg Budget-Tier',`${Number(data.avgBudgetTierScore||50).toFixed(0)}/100`,true),
    ...
    metric('Avg Nachfrage-Datenqualität',`${Number(data.avgDemandDataConfidence||0).toFixed(0)}/100`),
    ...
    metric('Avg Games-Signal',`${Number(data.avgFutbinGamesScore||0).toFixed(0)}/100`),
    metric('Avg Sales-Evidence',`${Number(data.avgFutbinSalesEvidence||0).toFixed(0)}/100`),
    metric('Avg Trade-Qualität',`${Number(data.avgTradeQuality).toFixed(0)}/100`,true),
    metric('A/A+ Karten',`${data.gradeACount} / ${data.count}`),
    metric('Avg Kapital-Effizienz',`${Number(data.avgCapitalEfficiency).toFixed(0)}/100`),
    metric('Avg Wiederholbarkeit',`${Number(data.avgRepeatability).toFixed(0)}/100`),
    metric('Avg Langfrist-Profit',`${Number(data.avgLongTermProfitScore).toFixed(0)}/100`,true),
    metric('Avg Trader-Wissen',`${Number(data.avgTraderPriorScore||0).toFixed(0)}/100`,true),
    metric('Avg ÜV-Methoden-Consensus',`${Number(data.avgTraderMethodConsensus||50).toFixed(0)}/100`),
    metric('Avg Sale-Likelihood-Index',`${Number(data.avgSaleLikelihoodIndex||0).toFixed(0)}/100`),
    metric('Avg Kapitalbindungsrisiko',`${Number(data.avgCapitalLockRisk||0).toFixed(0)}/100`),
    metric('Avg Ziel-Support',`${Number(data.avgTargetSupportScore||50).toFixed(0)}/100`,true),
    ...
    metric('Budget-Feasibility',`${Number(data.count||data.generatedCount||0) >= Number(data.requestedCount||data.count||0) && Number(data.totalBuy||0) <= Number(data.budget||0) ? 'OK Ziel erreicht' : data.dynamicCountReduced ? `OK ${data.count}/${data.requestedCount||data.count} sichere Slots` : data.candidatePipeline?.adaptiveBudgetFeasible===false ? 'Ziel nicht voll machbar' : '-'}`),
    metric('Qualitätsmodus',`${data.dynamicCountReduced?'QUALITY-FIRST | Slots reduziert':Number(data.requestedCount||data.count)===100?'100-SLOT':'BUDGET-SLOTS'}`,true),
    ...
    metric('Avg Turnover-Index',`${Number(data.avgTurnoverIndex).toFixed(0)}/100`),
    metric('Niedriges Risiko',`${data.lowRiskCount} / ${data.count}`),
    metric('Mit Lernhistorie',`${data.learningMatches} / ${data.count}`),
    metric('Avg Preissicherheit',`${Number(data.avgLearningScore).toFixed(0)}/100`),
...
function signalText(c){
  ...
  if(c.marketMover) out.push(`Mover ${Number.isFinite(c.marketMover.changePct)?`${c.marketMover.changePct>0?'+':''}${Number(c.marketMover.changePct).toFixed(1)}%`:'OK'}`);
  ...
  if(Number.isFinite(c.avgReportedRelists)) out.push(`Avg Relists ${Number(c.avgReportedRelists).toFixed(1)}`);
  ...
  if(c.riskFlags?.length) out.push(`WARN ${c.riskFlags[0]}`);
...
function renderDetail(c, pricing, status, quality){
  ...
  return `<div class="detailPanel">
    <div class="detailGrid">
      ...
      ${detailMetric('EA-Steuer 5 %',`${coins(pricing.tax)} Coins`)}
...
function renderRows(cards){
  ...
  rows.innerHTML = cards.map(c=>{
    ...
    const riskText = blocked ? 'BLOCK gesperrt' : quality.flags.length ? `WARN ${quality.flags.length} Risiko${quality.flags.length===1?'':'s'}` : 'OK niedrig';
